...
def utc_to_local(utc_dt):
    return utc_dt.replace(tzinfo=timezone.utc).astimezone(tz="Europe/Stockholm")
...
def get_bookmark(
    state, stream, default, bookmark_field=None, parent=None, parent_id=None
):
...
def write_bookmark(
    state, stream, value, bookmark_field=None, parent=None, parent_id=None
):
    if parent and parent_id:
        key = "{}(parent_{}_id:{})".format(bookmark_field, parent, parent_id)
    else:
    ...
    LOGGER.info(
        "Write state for Stream: {}, {} ID: {}, value: {}".format(
...
def process_records(
    ...
    with metrics.record_counter(stream_name) as counter:
        ...
        LOGGER.info(
            "Stream: {}, Processed {} records".format(stream_name, counter.value)
        )
...
def sync_endpoint(
    ...
    last_datetime = get_bookmark(
        state, stream_name, start_date, bookmark_field, parent, parent_id
    )
    max_bookmark_value = last_datetime
    ...
    while start_window < now_datetime:
        LOGGER.info(
            "START Sync for Stream: {}{}".format(
                ...
                ", Date window from: {} to {}".format(
                    start_window.date(), end_window.date()
                )
        ...
        for country_code in country_code_list:
            ...
            if stream_name.startswith("targeting_"):
                ...
                path = base_path.format(country_code=country_code, parent_id=parent_id)
                ...
                if "_stats_" in stream_name:
                    for data_record in data.get(data_key_array, []):
                        ...
                        for record in records:
                            ...
                            if (
                                "id" not in transformed_record
                                or "start_time" not in transformed_record
                            ):
                                LOGGER.error(
                                    "Stream: {}, Missing key (id or start_time)".format(
                                        stream_name
                                    )
                                )
                                LOGGER.error(
                                    "transformed_record: {}".format(transformed_record)
                                )
                                raise RuntimeError
                    ...
                    for child_stream_name, child_endpoint_config in children.items():
                        if child_stream_name in sync_streams:
                            ...
                            for record in transformed_data:
                                ...
                                LOGGER.info(
                                    "START Sync for Stream: {}, parent_stream: {}, parent_id: {}".format(
                                ...
                                LOGGER.info(
                                    "FINISHED Sync for Stream: {}, parent_id: {}, total_records: {}".format(
                                        child_stream_name,
                                        parent_id,
                                        child_total_records,
                                    )
...
def sync(client, config, catalog, state):
    ...
    for stream_name, stream_metadata in flat_streams.items():
        ...
        if stream_name in selected_streams:
            LOGGER.info(
                "stream: {}, parent: {}, grandparent: {}, great_grandparent: {}".format(
                    stream_name,
                    parent_stream,
                    grandparent_stream,
            ...
            if (
                great_grandparent_stream
                and great_grandparent_stream not in sync_streams
            ):
                sync_streams.append(grandparent_stream)
